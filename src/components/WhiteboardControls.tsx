...
const WhiteboardControls = ({ activeTab, handleActiveTabChange, setLineWidth, setLineColor, clearCanvas, drawGridLines, toggleEraser, isErasing }: Props) => {
  ...
  return (
    <div className="p-0 m-0 bg-dark text-light">
      <div className="d-flex align-items-center gap-1 border-bottom border-dark">
        {/* TABS */}
        <div className="gap-0">
        <button title="Click or press 1" className={`btn btn-sm ${activeTab === 1 ? 'btn-light' : 'btn-secondary'}`} onClick={() => handleActiveTabChange(1)}>1</button>
        <button title="Click or press 2" className={`btn btn-sm ${activeTab === 2 ? 'btn-light' : 'btn-secondary'}`} onClick={() => handleActiveTabChange(2)}>2</button>
        <button title="Click or press 1" className={`btn btn-sm ${activeTab === 3 ? 'btn-light' : 'btn-secondary'}`} onClick={() => handleActiveTabChange(3)}>3</button>
        </div>
